...
class _ProgressDashboardScreenState extends State<ProgressDashboardScreen>
  ...
  // Valores por defecto locales (se usarán solo si el Cubit aún no cargó datos)
  ...
  final List<double> _heartRateFallback = [72, 74, 95, 120, 110, 89, 75, 70];
  ...
  Widget _buildActivityStats() {
    ...
    final calories =
        snapshot?.today.caloriesBurned ?? _caloriesBurned;
    final steps = snapshot?.today.steps ?? _steps;
    final distance = snapshot?.today.distanceKm ?? _distance;
    final activeMinutes =
        ...
        children: [
          AnimatedBuilder(
            ...
            builder: (context, _) {
              return ActivityRingPieChart(
                title: 'Calorías',
                current: calories * _animation.value,
                target: 800,
          ...
          AnimatedBuilder(
            ...
            builder: (context, _) {
              return ActivityRingPieChart(
                title: 'Pasos',
                current: steps * _animation.value,
                target: 10000,
                color: Colors.blue,
                unit: 'pasos',
                size: 90,
          ...
          AnimatedBuilder(
            ...
            builder: (context, _) {
              return ActivityRingPieChart(
                title: 'Distancia',
                current: distance * _animation.value,
                target: 8,
                color: Colors.green,
                unit: 'km',
                size: 90,
          ...
          AnimatedBuilder(
            ...
            builder: (context, _) {
              return ActivityRingPieChart(
                title: 'Activo',
                current: activeMinutes * _animation.value,
                target: 60,
                color: Colors.amber,
                unit: 'min',
                size: 90,
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    _buildRingLegend(
                      'Movimiento',
                      '${(calories * 0.75).toInt()} / $calories kcal',
                    ...
                    _buildRingLegend(
                      'Ejercicio',
                      '${(activeMinutes * 0.62).toInt()} / $activeMinutes min',
  ...
  Widget _buildActivityTab() {
    ...
    final snapshot = progressState.snapshot;
    final heartRate =
        snapshot?.heartRateBpm ?? _heartRateFallback;
                      ...
                      bottomTitles: AxisTitles(
                        sideTitles: SideTitles(
                          ...
                          getTitlesWidget: (value, meta) {
                            ...
                            if (value % 1 == 0 &&
                                value.toInt() %
                                        ((heartRate.length / 6).ceil()) ==
                                    0) {
                              final index =
                                  value ~/ ((heartRate.length / 6).ceil());
                      ...
                      LineChartBarData(
                        spots: _animation.value < 1
                          ? List<FlSpot>.generate(
                              (heartRate.length * _animation.value)
                                  .toInt()
                                  .clamp(1, heartRate.length),
                              (i) => FlSpot(
                                i.toDouble(),
                                heartRate[i],
                              ),
  ...
  Widget _buildProgressTab() {
    ...
    final snapshot = progressState.snapshot;
    final weekly =
        snapshot?.weeklyWorkoutHours ?? _weeklyWorkouts;
    final totalHours = weekly.fold<double>(0, (sum, h) => sum + h);
    final averagePerDay = weekly.isEmpty ? 0 : totalHours / weekly.length;
    final bestDayIndex =
        weekly.isEmpty ? 0 : weekly.indexWhere((h) => h == weekly.reduce(math.max));
    const dayLabels = ['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo'];
    final bestDayLabel = dayLabels[bestDayIndex.clamp(0, dayLabels.length - 1)];
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                _buildWeeklyStat(
                  'Media',
                  '${averagePerDay.toStringAsFixed(1)}h/día',
                  Icons.calendar_today,
                ),
                const SizedBox(width: 24),
                _buildWeeklyStat('Mejor', bestDayLabel, Icons.emoji_events),
              ],
            ),
            ...
            Center(
              child: TextButton(
                onPressed: () {
                  context.pushNamed('fitness-tracker');
                },
                child: const Text(
                  'Ver detalles en Fitness Tracker',
                  style: TextStyle(
